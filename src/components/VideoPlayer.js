...
const VideoPlayer = () => {
  ...
  const handleEpisodeChange = (episode) => {
    ...
    setIsLoading(true);
    addToWatchedList(
      `${videoPlayerState.type}:${item.id}:${getTitle(item)}:${selectedSeason}:${episode}`,
    );
  ...
  return (
    <div className='player-video-player-overlay'>
      ...
      <iframe
        ...
        onError={() => {
          setError(
            'Failed to load video. Please try another source or server.',
          );
          setIsLoading(false);
        }}
      ></iframe>
      <div
        className={`player-overlay-menu-button ${showOverlay ? 'active' : ''}`}
        onClick={toggleOverlay}
      >
        {showOverlay ? (
          <FaTimes size={24} title='Close This Menu' />
        ) : (
          <FaEllipsisH size={24} title='Settings' />
        )}
      </div>
      {showOverlay && (
        <div className='player-overlay-menu'>
          ...
          <div className='player-server-switch-container'>
            <span className='player-server-switch-label'>
              Video troubles? Try switching server:
            </span>
          ...
          <button
            className='player-control-button player-close-button'
            onClick={hidePlayer}
          >
            Close Video
          </button>
