...
const NameParticles = ({ text = 'Youvies', logoSrc }) => {
  ...
  const captureParticleCoordinates = (
    textX,
    textY,
    textWidth,
    textHeight,
    ...
    for (let y = 0; y < textCoordinates.height; y++) {
      for (let x = 0; x < textCoordinates.width; x++) {
        ...
        if (alpha > 128) {
          ...
          let color = 'white';
          particleText.current.push(
            new Particle(positionX, positionY, 3, color),
          );
        }
      }
    }
    ...
    if (logoX !== null && logoY !== null && logoWidth > 0 && logoHeight > 0) {
      const logoCoordinates = ctx.getImageData(
        logoX,
        logoY,
        logoWidth,
        logoHeight,
      ...
      for (let y = 0; y < logoCoordinates.height; y++) {
        for (let x = 0; x < logoCoordinates.width; x++) {
          const alpha =
            logoCoordinates.data[y * 4 * logoCoordinates.width + x * 4 + 3];
          if (alpha > 128) {
