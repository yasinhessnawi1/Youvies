...
export const fetchOneItem = async (category, id, type) => {
  switch (category) {
    case 'movies':
      return await fetchMovieDetails(id).then((movie) => ({
        ...movie,
        type: 'movies',
      }));
    case 'shows':
      return await fetchShowDetails(id).then((show) => ({
        ...show,
        type: 'shows',
      }));
    case 'anime':
      return await fetchOneAnime(id).then((anime) => ({
        ...anime,
        type: 'anime',
      }));
    case 'home':
  ...
  switch (category) {
    ...
    case 'anime':
      return await searchAnime(title).then((anime) =>
        anime.map((anime) => ({ ...anime, type: 'anime' })),
      );
    case 'home':
